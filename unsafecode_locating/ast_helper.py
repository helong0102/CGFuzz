...
from unsafecode_locating.ast_walker import AstWalker
from unsafecode_locating.utils import run_command


class AstHelper:
    def __init__(self, filename, input_type="solidity", remap="", allow_paths=""):
    ...
    def extract_function_definitions(self, c_name):
        node = self.contracts["contractsByName"][c_name]
        functions = []
        if node:
            base_contracts = self.get_linearized_base_contracts(node["id"], self.contracts["contractsById"])
    ...
    def extract_functions_definitions(self):
        ret = {}
        for contract in self.contracts["contractsById"]:
            name = self.contracts["contractsById"][contract]["attributes"]["name"]
            source = self.contracts["sourcesByContract"][contract]
            full_name = source + ":" + name
            ret[full_name] = self.extract_function_definitions(full_name)
        return ret

    def get_contract_names(self):
        contract_names = []
        contracts_by_name = self.contracts['contractsByName']
        for key, _ in contracts_by_name.items():
            contract_names.append(key.split(":")[-1])
        return contract_names

    def get_func_name_to_def_nodes(self, c_name):
        func_name_to_def_nodes = {}
        node = self.contracts['contractsByName'][c_name]
        walker = AstWalker()
        func_def_nodes = []
        if node:
        ...
        for func_def_node in func_def_nodes:
            ...
            func_name_to_def_nodes[func_name] = func_def_node

        return func_name_to_def_nodes
